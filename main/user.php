...
$exp_array = array(100, 200, 300, 400, 500, 600, 700, 800, 900);
...
if ($connect->connect_error) {
    ...
    $reply_num = $result['reply_num'];
    $video_num = $result['video_num'];
    $post_score_total = $result['post_score'];
    $article_score = $result['article_score'];
    $reply_score_total = $result['reply_score'];
    $video_score_total = $result['video_score'];
    $select_sign = mysqli_fetch_assoc(mysqli_query($connect, "select * from sign where user_id = '".$id."'"));
    $point = $select_sign['point'];
}

$level = ($exp / 100) == $level ? ($exp / 100) : $level;
$update_user_level = mysqli_query($connect, "update user set level = '".$level."' where id = $id");
...
$user_number_error = $user_birthday_error = "";
if (isset($_POST['submit'])) {
    if (!empty($_POST['name'])) {
        $user_name = $_POST['name'];
        $update_user_name = "update user set name = '".$user_name."' where id = '".$id."' ";
        $update_user_name_result = mysqli_query($connect, $update_user_name);
        if (!$update_user_name_result) {
            die("修改昵称异常：" . mysqli_error($connect));
        } else {
            header("Refresh: 0");
        }
    }
    if (!empty($_POST['number'])) {
        $firstFilter_number = test_input($_POST['number']);
        if (isPhoneNumber($firstFilter_number)) {
            $user_number = $firstFilter_number;
            $update_user_number = "update user set number = '".$user_number."' where id = '".$id."'";
            $update_user_number_result = mysqli_query($connect, $update_user_number);
            $_SESSION['number'] = $user_number;
            if (!$update_user_number_result) {
                die("修改号码异常：" . mysqli_error($connect));
            } else {
                header("Refresh: 0");
            }
        } else {
            $user_number_error = "手机号码格式不正确";
        }
    }
    if (!empty($_POST['sex'])) {
        $sex = $_POST['sex'];
        $update_user_sex = "update user set sex = '".$sex."' where id = '".$id."'";
        $update_user_sex_result = mysqli_query($connect, $update_user_sex);
        if (!$update_user_sex_result) {
            die("修改性别异常：" . mysqli_error($connect));
        } else {
            header("Refresh: 0");
        }
    }
    if (!empty($_POST['email'])) {
        ...
        $update_user_email = "update user set email = '".$user_email."' where id = '".$id."'";
        $update_user_email_result = mysqli_query($connect, $update_user_email);
        if (!$update_user_email_result) {
            die("修改邮箱异常：" . mysqli_error($connect));
        } else {
            header("Refresh: 0");
        }
    }
    if (!empty($_POST['birthday'])) {
        $is_date = strtotime($_POST['birthday'] ? strtotime($_POST['birthday']) : false);
        if (!$is_date) {
            $user_birthday_error = "请输入正确的生日哦";
        } else {
            $user_birthday = $_POST['birthday'];
            $update_user_birthday = "update user set birthday = '".$user_birthday."' where id = '".$id."'";
            $update_user_birthday_result = mysqli_query($connect, $update_user_birthday);
            if (!$update_user_birthday_result) {
                die("修改生日异常：" . mysqli_error($connect));
            } else {
                header("Refresh: 0");
            }
        }
    ...
    if ($_FILES['file']['size'] > 0) {
        $allowedExts = array("gif", "jpeg", "jpg", "png", "svg", "bmp", "x-png");
        ...
        if ((($_FILES["file"]["type"] == "image/gif")
            ...
            && ($_FILES["file"]["size"] < 2048000000)   // 小于 200 kb
            && in_array($extension, $allowedExts)) {
            if ($_FILES["file"]["error"] > 0) {
                echo "错误：: " . $_FILES["file"]["error"] . "<br>";
            } else {
                $image_temp = explode("/", $_FILES["file"]["type"]);
                $image_type = end($image_temp);
                $image_name = str_replace(' ', '_', $name) . "." . $image_type;
                $new_image_path = "./user_image/" . $image_name;
                $time = date("Y-m-d H:i:s", mktime());
                if (copy($_FILES["file"]["tmp_name"], $new_image_path)) {
                    if ($image_path == "./user_image/") {
                        $insert_user_image = "insert into image (image_name, image_path, image_time) values ('".$image_name."', '".$new_image_path."', '".$time."')";
                        $update_user_image = "update user set image_path = '".$new_image_path."' where id = '".$id."'";
                        $update_user_image_result = mysqli_query($connect, $update_user_image);
                        if (!mysqli_query($connect, $insert_user_image) || !$update_user_image_result) {
                            die("头像上传异常：" . mysqli_error($connect));
                        } else {
                            header("Refresh: 0");
                        }
                    } else {
                        $update_user_image = "update user set image_path = '".$new_image_path."' where id = '".$id."'";
                        if (!mysqli_query($connect, $update_user_image)) {
                            die("上传头像异常：" . mysqli_error($connect));
                        } else {
                            header("Refresh: 0");
                        }
                    }
...
<header id="tt-header" style="background-color: #f5dc7d; -webkit-border-bottom-left-radius: 15px; -webkit-border-bottom-right-radius: 15px;  box-shadow: 2px 2px 20px 20px rgba(0, 0, 0, 0.15); border: #1cbbb4">
    <div class="container">
        <div class="row tt-row no-gutters">
            <div class="col-auto">
                ...
                <div class="tt-logo">
                    <a href="../index.php"><img src="../pic/logo.jpg" alt="" class="image"></a>
                </div>
                <div class="tt-desktop-menu">
                    <nav>
                        <ul>
                            <li><a href="../index.php"><span>首页</span></a></li>
                            <li><a href="file.php"><span>佑教学</span></a></li>
                            <li><a href="chat.php"><span>佑交流</span></a></li>
                            <li><a href="knowledge_lib.php"><span>佑知识</span></a>
                                <ul style="opacity: 0.85; background-color: #f5dc7d; -webkit-border-bottom-left-radius: 15px; -webkit-border-bottom-right-radius: 15px; box-shadow: 2px 2px 20px 20px rgba(0, 0, 0, 0.15); border: #1cbbb4; ">
                                    <li><a href="knowledge_frame.php">学习框架</a></li>
                                    <li><a href="../page-exams/page-exam-all.php"><span>编程小测</span></a> </li>
                                    <li><a href="../page-books/page-books.php"><span>编程书目</span></a> </li>
                                </ul>
                            </li>
                        </ul>
                    </nav>
                ...
                <div class="tt-user-info d-flex justify-content-center">
                    <div class="tt-avatar-icon tt-size-md">
                        <a href="" class="tt-btn-icon">
                            ...
                            <?php } else { ?>
                                <img src="<?php echo $image_path;  ?>" alt="" class="image">
                            <?php } ?>
                        </a>
                    </div>
                </div>
            ...
            <div class="tt-user-header">
                ...
                <div class="tt-col-title">
                    ...
                    <ul class="tt-list-badge">
                        <?php if ($identity == 0) {$identity_virtual = "普通会员"; } else {$identity_virtual = "创作者"; } ?>
                        <li><span class="tt-color03 tt-badge">身份：<?php echo $identity_virtual; ?></span> </li>
                        <?php if ($identity == 0) { ?>
                        <li><a href="up_identity.php"><small>成为创作者？</small></a> </li>
                        <?php } ?>
                        <li><span class="tt-color15 tt-badge">Lv: <?php echo $level + 1; ?></span></li>
                        <li><span class="tt-color20 tt-badge">升级还需经验：<?php echo $exp_array[$level] - $exp; ?></span> </li>
                        <li><span class="tt-color12 tt-badge">佑币余额：<?php echo $point; ?></span></li>
                        <li><a href="deposit.php"><small>充值佑币?</small></a></li>
                    </ul>
                </div>
        ...
        <div class="tt-tab-wrapper">
            <div class="tt-wrapper-inner">
                <ul class="nav nav-tabs pt-tabs-default" role="tablist">
                    <li class="nav-item">
                        <a class="nav-link" data-toggle="tab" href="#tt-tab-07" role="tab"><span>收藏的问题</span></a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" data-toggle="tab" href="#tt-tab-08" role="tab"><span>收藏的文章</span></a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" data-toggle="tab" href="#tt-tab-09" role="tab"><span>收藏的视频</span></a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" data-toggle="tab" href="#tt-tab-01" role="tab"><span>提出的问题</span></a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" data-toggle="tab" href="#tt-tab-02" role="tab"><span>创作的文章</span></a>
                    </li>
                    <?php if ($identity == 1) {?>
                    <li class="nav-item">
                        <a class="nav-link" data-toggle="tab" href="#tt-tab-03" role="tab"><span>发布的视频</span></a>
                    </li>
                    <?php } ?>
                    <li class="nav-item">
                        <a class="nav-link" data-toggle="tab" href="#tt-tab-04" role="tab"><span>问题收到的回复</span></a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" data-toggle="tab" href="#tt-tab-05" role="tab"><span>文章收到的回复</span></a>
                    </li>
                    <!--TODO:根据成员等级判断是否出现-->
                    <?php if ($identity == 1) { ?>
                    <li class="nav-item">
                        <a class="nav-link" data-toggle="tab" href="#tt-tab-06" role="tab"><span>视频收到的回复</span></a>
                    </li>
                    <?php } ?>
                </ul>
            </div>
            ...
            <div class="tab-content">
                <!--收藏的问题-->
                <?php
                $select_post_fav = mysqli_query($connect, "select * from fav_post where fav_post_owner_number = $number order by post_frequency desc");
                if (!$select_post_fav) {
                    die("收藏的问题读取失败：" . mysqli_error($connect));
                } else {
                ?>
                <div class="tab-pane tt-indent-none show-active" id="tt-tab-07" role="tabpanel">
                    <div class="tt-topic-list">
                        <div class="tt-list-header">
                            <div class="tt-col-topic">主题</div>
                            <div class="tt-col-value-large hide-mobile">标签</div>
                            <div class="tt-col-value-large hide-mobile">点赞数</div>
                            <div class="tt-col-value-large hide-mobile">回复数</div>
                            <div class="tt-col-value-large hide-mobile">赞赏数</div>
                            <div class="tt-col-value-large hide-mobile">收藏数</div>
                            <div class="tt-col-value-large">时间</div>
                        </div>
                        <?php
                        while ($row = mysqli_fetch_array($select_post_fav)) {
                            $post_title_fav = $row['post_title'];
                            $post_owner_id = $row['post_owner_id'];
                            $post_info = mysqli_fetch_assoc(mysqli_query($connect, "select * from post where post_title = '".$post_title_fav."' and post_owner_id = '".$post_owner_id."'"));
                            $post_tag_fav = $post_info['post_tag'];
                            $post_reply_num_fav = $post_info['post_reply_num'];
                            $post_score_fav = $post_info['post_score'];
                            $post_money_fav = $post_info['post_money'];
                            $post_fav_num = $post_info['post_fav_num'];
                            $post_time_fav = $post_info['post_time'];
                            ?>
                            <div class="tt-item">
                                <div class="tt-col-avatar">
                                    <svg class="tt-icon">
                                ...
                                <div class="tt-col-description">
                                    <h6 class="tt-title">
                                        <a href="post.php?post_title=<?php echo $post_title_fav; ?>&post_owner_id=<?php echo $post_owner_id; ?>">
                                            <?php echo $post_title_fav; ?>
                                        </a>
                                    </h6>
                                    <div class="row align-items-center no-gutters">
                                        <div class="col-1 ml-auto show-mobile">
                                            <div class="tt-value">
                                                <?php echo $post_time_fav; ?>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="tt-col-value-large hide-mobile">
                                <span class="tt-badge tt-color20">
                                    <?php echo $post_tag_fav; ?>
                                </span>
                                </div>
                                <div class="tt-col-value-large hide-mobile">
                                    <?php echo $post_score_fav; ?>
                                </div>
                                <div class="tt-col-value-large tt-color-select  hide-mobile">
                                    <?php echo $post_reply_num_fav; ?>
                                </div>
                                <div class="tt-col-value-large hide-mobile">
                                    <?php echo $post_money_fav; ?>
                                </div>
                                <div class="tt-col-value-large hide-mobile">
                                    <?php echo $post_fav_num; ?>
                                </div>
                                <div class="tt-col-value-large">
                                    <?php echo $post_time_fav; ?>
                                </div>
                            </div>
                        <?php }
                ...
                <!--收藏的文章-->
                <?php
                $select_article_fav = mysqli_query($connect, "select * from fav_article where fav_article_owner_number = $number order by article_frequency desc");
                if (!$select_article_fav) {
                    die("收藏的文章读取失败：" . mysqli_error($connect));
                } else {
                ?>
                <div class="tab-pane tt-indent-none" id="tt-tab-08" role="tabpanel">
                    <div class="tt-topic-list">
                        <div class="tt-list-header">
                            <div class="tt-col-topic">主题</div>
                            <div class="tt-col-value-large hide-mobile">标签</div>
                        ...
                        <?php
                        while ($row = mysqli_fetch_array($select_article_fav)) {
                            $article_title_fav = $row['article_title'];
                            $article_owner_id = $row['article_owner_id'];
                            $article_info = mysqli_fetch_assoc(mysqli_query($connect, "select * from article where article_title = '".$article_title_fav."' and article_owner_id = '".$article_owner_id."'"));
                            $article_tag_fav = $article_info['article_tag'];
                            $article_reply_num_fav = $article_info['article_reply_num'];
                            $article_score_fav = $article_info['article_score'];
                            $article_money_fav = $article_info['article_money'];
                            $article_fav_num = $article_info['article_fav_num'];
                            $article_time_fav = $article_info['article_time'];
                            ?>
                            <div class="tt-item">
                                <div class="tt-col-avatar">
                                    <svg class="tt-icon">
                                ...
                                <div class="tt-col-description">
                                    <h6 class="tt-title">
                                        <a href="<?php echo "article.php?article_title=" . $article_title_fav ?>&article_owner_id=<?php echo $article_owner_id; ?>">
                                            <?php echo $article_title_fav; ?>
                                        </a>
                                    </h6>
                                    <div class="row align-items-center no-gutters">
                                        <div class="col-1 ml-auto show-mobile">
                                            <div class="tt-value">
                                                <?php echo $article_time_fav; ?>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="tt-col-value-large hide-mobile">
                                <span class="tt-badge tt-color18">
                                    <?php echo $article_tag_fav; ?>
                                </span>
                                </div>
                                <div class="tt-col-value-large hide-mobile">
                                    <?php echo $article_score_fav; ?>
                                </div>
                                <div class="tt-col-value-large tt-color-select  hide-mobile">
                                    <?php echo $article_reply_num_fav; ?>
                                </div>
                                <div class="tt-col-value-large hide-mobile">
                                    <?php echo $article_money_fav; ?>
                                </div>
                                <div class="tt-col-value-large hide-mobile">
                                    <?php echo $article_fav_num; ?>
                                </div>
                                <div class="tt-col-value-large">
                                    <?php echo $article_time_fav; ?>
                                </div>
                ...
                <!--收藏的视频-->
                <?php
                $select_video_fav = mysqli_query($connect, "select * from fav_video where fav_video_owner_number = '".$number."'");
                if (!$select_video_fav) {
                    die("收藏的视频读取失败：" . mysqli_error($connect));
                } else { ?>
                <div class="tab-pane tt-indent-none" id="tt-tab-09" role="tabpanel">
                    <div class="tt-topic-list">
                        <div class="tt-list-header">
                            <div class="tt-col-topic">主题</div>
                            <div class="tt-col-value-large hide-mobile">标签</div>
                        ...
                        <?php
                        while ($row = mysqli_fetch_array($select_video_fav)) {
                            $video_title_fav = $row['video_title'];
                            $video_owner_id = $row['video_owner_id'];
                            $video_info = mysqli_fetch_assoc(mysqli_query($connect, "select * from video where video_title = '".$video_title_fav."' and video_owner_id = '".$video_owner_id."'"));
                            $video_tag_fav = $video_info['video_tag'];
                            $video_reply_num_fav = $video_info['video_reply_num'];
                            $video_score_fav = $video_info['video_score'];
                            $video_money_fav = $video_info['video_money'];
                            $video_fav_num = $video_info['video_fav_num'];
                            $video_time_fav = $video_info['video_time'];
                            ?>
                            <div class="tt-item">
                                <div class="tt-col-avatar">
                                    <svg class="tt-icon">
                                ...
                                <div class="tt-col-description">
                                    <h6 class="tt-title"><a
                                                href="video.php?video_title=<?php echo $video_title_fav; ?>&video_owner_id=<?php echo $video_owner_id; ?>">
                                            <?php echo $video_title_fav; ?>
                                        </a></h6>
                                    <div class="row align-items-center no-gutters">
                                        <div class="col-1 ml-auto show-mobile">
                                            <div class="tt-value">
                                                <?php echo $video_time_fav; ?>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="tt-col-value-large hide-mobile">
                                <span class="tt-badge tt-color18">
                                    <?php echo $video_tag_fav; ?>
                                </span>
                                </div>
                                <div class="tt-col-value-large hide-mobile">
                                    <?php echo $video_score_fav; ?>
                                </div>
                                <div class="tt-col-value-large tt-color-select  hide-mobile">
                                    <?php echo $video_reply_num_fav; ?>
                                </div>
                                <div class="tt-col-value-large hide-mobile">
                                    <?php echo $video_money_fav; ?>
                                </div>
                                <div class="tt-col-value-large hide-mobile">
                                    <?php echo $video_fav_num; ?>
                                </div>
                                <div class="tt-col-value-large">
                                    <?php echo $video_time_fav; ?>
                                </div>
                ...
                <!--帖子-->
                <?php
                $sql_select = "select * from post where post_owner_id = $id order by `post_id` desc ";
                $result = mysqli_query($connect, $sql_select);
                if (!$result) {
                    die("数据读取失败: " . mysqli_error($connect));
                } else { ?>
                    ...
                    <div class="tt-topic-list">
                        ...
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            <div class="tt-item">
                                ...
                                <div class="tt-col-description">
                                    <h6 class="tt-title">
                                        <a href="post.php?post_title=<?php echo $post_title; ?>&post_owner_id=<?php echo $id; ?>">
                                            <?php echo $post_title; ?>
                                        </a>
                                    </h6>
                                    <div class="row align-items-center no-gutters">
                                ...
                                <div class="tt-col-value-large hide-mobile">
                                    <span class="tt-badge tt-color08">
                                        <?php echo $post_tag; ?>
                                    </span>
                                </div>
                                <div class="tt-col-value-large hide-mobile">
                ...
                <!--文章-->
                <?php
                $select_article = "select * from article where article_owner_id = $id order by article_id desc";
                $select_article_result = mysqli_query($connect, $select_article);
                if (!$select_article_result) {
                    die("文章数据读取失败：" . mysqli_error($connect));
                } else { ?>
                <div class="tab-pane tt-indent-none" id="tt-tab-02" role="tabpanel">
                    <div class="tt-topic-list">
                        ...
                        <?php
                        while ($row = mysqli_fetch_assoc($select_article_result)) {
                            $article_title = $row['article_title'];
                            $article_tag = $row['article_tag'];
                            $article_reply_num = $row['article_reply_num'];
                            $article_score = $row['article_score'];
                            $article_money = $row['article_money'];
                            $article_fav_num = $row['article_fav_num'];
                            ...
                            <div class="tt-item">
                                ...
                                <div class="tt-col-description">
                                    <h6 class="tt-title">
                                        <a href="<?php echo "article.php?article_title=" . $article_title ?>&article_owner_id=<?php echo $id; ?>">
                                ...
                                <div class="tt-col-value-large hide-mobile">
                                    <span class="tt-badge tt-color16">
                                        <?php echo $article_tag; ?>
                                    </span>
                                </div>
                                <div class="tt-col-value-large hide-mobile">
                                    <?php echo $article_score; ?>
                                </div>
                                <div class="tt-col-value-large tt-color-select  hide-mobile">
                                    <?php echo $article_reply_num; ?>
                                </div>
                                <div class="tt-col-value-large hide-mobile">
                                    <?php echo $article_money; ?>
                                </div>
                                <div class="tt-col-value-large hide-mobile">
                                    <?php echo $article_fav_num; ?>
                                </div>
                ...
                <!--视频-->
                <?php
                $sql_select = "select * from video where video_owner_id = $id order by `video_id` desc ";
                $result = mysqli_query($connect, $sql_select);
                if (!$result) {
                    die("数据读取失败: " . mysqli_error($connect));
                } else { ?>
                ...
                <div class="tab-pane tt-indent-none" id="tt-tab-03" role="tabpanel">
                    <div class="tt-topic-list">
                        <div class="tt-list-header">
                            ...
                            <div class="tt-col-value-large hide-mobile">标签</div>
                            <div class="tt-col-value-large hide-mobile">点赞数</div>
                            <div class="tt-col-value-large hide-mobile">浏览量</div>
                            <div class="tt-col-value-large hide-mobile">赞赏数</div>
                            <div class="tt-col-value-large hide-mobile">收藏数</div>
                            <div class="tt-col-value-large">时间</div>
                        </div>
                        ...
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            $video_owner_id = $row['video_owner_id'];
                            $video_visit_num = $row['visit_num'];
                            $video_score = $row['video_score'];
                            $video_money = $row['video_money'];
                            $video_fav_num = $row['video_fav_num'];
                            ...
                            <div class="tt-item">
                                ...
                                <div class="tt-col-description">
                                    <h6 class="tt-title"><a href="video.php?video_title=<?php echo $video_title; ?>&video_owner_id=<?php echo $video_owner_id; ?>">
                                ...
                                <div class="tt-col-value-large hide-mobile">
                                    <span class="tt-badge tt-color05">
                                        <?php echo $video_tag; ?>
                                    </span>
                                </div>
                                <div class="tt-col-value-large hide-mobile">
                                    <?php echo $video_score; ?>
                                </div>
                                <div class="tt-col-value-large hide-mobile">
                                    <?php echo $video_visit_num; ?>
                                </div>
                                <div class="tt-col-value-large tt-color-select  hide-mobile">
                                    <?php echo $video_money; ?>
                                </div>
                                <div class="tt-col-value-large hide-mobile">
                                    <?php echo $video_fav_num; ?>
                                </div>
                ...
                <!--问题的回复-->
                <?php
                $select_reply = "select * from reply where reply_object_id = $id order by reply_id desc";
                $select_reply_result = mysqli_query($connect, $select_reply);
                if (!$select_reply_result) {
                    die("数据读取失败：" . mysqli_error($connect));
                } else { ?>
                ...
                <div class="tab-pane tt-indent-none" id="tt-tab-04" role="tabpanel">
                    <div class="tt-topic-list">
                        <div class="tt-list-header">
                            <div class="tt-col-topic">主题</div>
                            <div class="tt-col-value hide-mobile">回复者</div>
                            <div class="tt-col-value">时间</div>
                        </div>
                        <?php
                        while ($row = mysqli_fetch_assoc($select_reply_result)) {
                            $reply_object_id = $row['reply_object_id'];
                            $reply_owner_name = $row['reply_owner_name'];
                            $reply_post_title = $row['reply_post_title'];
                        ...
                        <div class="tt-item">
                            ...
                            <div class="tt-col-description">
                                <h6 class="tt-title">
                                    <a href="post.php?post_title=<?php echo $reply_post_title; ?>&post_owner_id=<?php echo $id; ?>">
                                        <?php echo $reply_post_title; ?>
                                    </a>
                                </h6>
                                <div class="row align-items-center no-gutters hide-desktop">
                ...
                <!--文章的回复-->
                <?php
                $select_a_reply = "select * from a_reply where a_reply_object_id = $id order by a_reply_id desc";
                $select_a_reply_result = mysqli_query($connect, $select_a_reply);
                if (!$select_a_reply_result) {
                    die("读取文章回复失败：" . mysqli_error($connect));
                } else { ?>
                ...
                <div class="tab-pane tt-indent-none" id="tt-tab-05" role="tabpanel">
                    <div class="tt-topic-list">
                        <div class="tt-list-header">
                            <div class="tt-col-topic">主题</div>
                            <div class="tt-col-value">回复者</div>
                            <div class="tt-col-value">时间</div>
                        </div>
                        <?php
                        while ($row = mysqli_fetch_assoc($select_a_reply_result)) {
                        $a_reply_object_id = $row['a_reply_object_id'];
                        $a_reply_owner_name = $row['a_reply_owner_name'];
                        $a_reply_article_title = $row['a_reply_article_title'];
                        $a_reply_content = $row['a_reply_content'];
                        $a_reply_time = $row['a_reply_time'];
                        ?>
                        <div class="tt-item">
                            <div class="tt-col-avatar">
                                <svg class="tt-icon">
                            ...
                            <div class="tt-col-description">
                                <h6 class="tt-title"><a href="article.php?article_title=<?php echo $a_reply_article_title; ?>&article_owner_id=<?php echo $a_reply_object_id; ?>">
                                        <?php echo $a_reply_article_title; ?>
                                    </a></h6>
                                <div class="row align-items-center no-gutters hide-desktop">
                                    <div class="col-3 ml-auto show-mobile">
                                        <div class="tt-value">
                                            <?php echo $a_reply_time; ?>
                                        </div>
                                    </div>
                                </div>
                                <div class="tt-content">
                                    <?php echo $a_reply_content; ?>
                                </div>
                            </div>
                            <div class="tt-col-value">
                                <span class="tt-badge tt-color08">
                                    <?php echo $a_reply_owner_name; ?>
                                </span>
                            </div>
                ...
                <!--视频的回复-->
                <?php
                $select_v_reply = "select * from v_reply where v_reply_object_id = $id order by v_reply_id desc";
                $select_v_reply_result = mysqli_query($connect, $select_v_reply);
                if (!$select_v_reply_result) {
                    die("读取视频回复失败：" . mysqli_error($connect));
                } else { ?>

                <div class="tab-pane tt-indent-none" id="tt-tab-06" role="tabpanel">
                    <div class="tt-topic-list">
                        <div class="tt-list-header">
                            <div class="tt-col-topic">主题</div>
                            <div class="tt-col-value">时间</div>
                        ...
                        <?php
                        while ($row = mysqli_fetch_assoc($select_v_reply_result)) {
                            $v_reply_object_id = $row['v_reply_object_id'];
                            $v_reply_video_title = $row['v_reply_video_title'];
                            $v_reply_content = $row['v_reply_content'];
                            $v_reply_time = $row['v_reply_time'];
                            ?>
                            <div class="tt-item">
                                <div class="tt-col-avatar">
                                    <svg class="tt-icon">
                                ...
                                <div class="tt-col-description">
                                <h6 class="tt-title"><a href="video.php?video_title=<?php echo $v_reply_video_title; ?>&video_owner_id=<?php echo $v_reply_object_id; ?>">
                                            <?php echo $v_reply_video_title; ?>
                                        </a></h6>
                                    <div class="row align-items-center no-gutters hide-desktop">
                                        <div class="col-3 ml-auto show-mobile">
                                            <div class="tt-value">
                                                <?php echo $v_reply_time; ?>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="tt-content">
                                        <?php echo $v_reply_content; ?>
                                    </div>
                                </div>
                                <div class="tt-col-value-large hide-mobile">
                                    <?php echo $v_reply_time; ?>
    ...
    <form class="form-default" method="post" action="" enctype="multipart/form-data">
        <div class="tt-form-upload">
            <div class="row no-gutter">
                <div class="col-auto ml-auto">
                    <label for="file">上传头像：（上传之后可能会有显示延迟）</label>
        ...
        <div class="form-group">
            ...
            <input class="form-control" id="loginUserName" type="text" name="name" placeholder="<?php echo $name;?>">
        </div>
        <div class="form-group">
            <label for="loginUserNumber">手机号: </label>
            <input class="form-control" id="loginUserNumber" type="tel" name="number" placeholder="<?php echo $number;?>">
            <span class="error"><?php echo "$user_number_error";?></span>
        </div>
        <?php
        if ($sex == 'female') { ?>
            <div class="form-group">
        ...
        <div class="form-group">
            ...
            <select id="sex" class="form-control" name="sex" required>
                <option value="male">男</option>
                <option value="female">女</option>
                <option value="unknown" selected>保密</option>
            </select>
        </div>
        <?php } ?>
        <div class="form-group">
            <label for="loginUserEmail">邮箱地址: </label>
            <input class="form-control" id="loginUserEmail" type="email" name="email">
        </div>
        <div class="form-group">
            <label for="birthday">生日: </label>
            <input class="form-control" id="birthday" type="text" name="birthday">
            <span class="choice"><?php echo "$user_birthday_error";?></span>
        </div>
        <div class="form-group">
            <a href="../login/retrieve_passwd.php">修改密码</a>
        </div>
        <div class="form-group">
            <button type="submit" name="submit" class="btn btn-secondary btn-block" style="border-radius:25px; -moz-border-radius:25px; height: 50px">保存</button>
        </div>
    </form>
</div>

<!--发帖上传-->
<a href="add_post.php" class="tt-btn-create-topic">
    <span class="tt-icon">
        <svg>
