...
if ($connect->connect_error) {
    ...
    $select_article_owner = mysqli_fetch_assoc(mysqli_query($connect, "select name, image_path from user where id = $article_owner_id"));
    $article_owner_name = $select_article_owner['name'];
    $article_owner_image_path = $select_article_owner['image_path'];
    ...
    $select_article = "select * from article where article_title = '".$article_title."' and article_owner_id = $article_owner_id";
    $select_article_result = mysqli_query($connect, $select_article);
    if (!$select_article_result) {
        die("读取帖子信息失败：" . mysqli_error($connect));
    } else {
        $result = mysqli_fetch_assoc($select_article_result);
        $article_id = $result['article_id'];
        $content = $result['article_content'];
        $reply_num = $result['article_reply_num'];
        $score = $result['article_score'];
        $dis_score = $result['article_dis_score'];
        $money = $result['article_money'];
        $time = $result['article_time'];
        $fav_num = $result['article_fav_num'];
    }
}
?>
...
<head>
    <meta charset="UTF-8">
    <title><?php echo $_GET['article_title']; ?></title>
...
if (isset($_POST['score'])) {
    $update_article_score = "update `article` set `article_score` = (`article_score` + 1) where `article_id` = $article_id";
    $update_article_score_result = mysqli_query($connect, $update_article_score);
    $update_user = "update `user` set `article_score` = (`article_score` + 1) where `number` = $number";
    $update_user_result = mysqli_query($connect, $update_user);
    header("Refresh: 0");
}

if (isset($_POST['dis_score'])) {
    $update_article_dis_score = "update article set article_dis_score = (article_dis_score + 1) where article_id = $article_id";
    $update_article_dis_score_result = mysqli_query($connect, $update_article_dis_score);
    header("Refresh: 0");
}

if (isset($_POST['reply'])) {
    if (!empty($_POST['message'])) {
        $a_reply_message = $_POST['message'];
        $time = date("Y-m-d H:i:s", mktime());
        $insert_reply = "insert into a_reply (article_id, a_reply_object_id , a_reply_owner_id, a_reply_owner_name, a_reply_article_title, a_reply_content, a_reply_time, a_reply_score, a_reply_dis_score) 
        VALUES ('".$article_id."', '".$article_owner_id."', '".$user_id."', '".$user_name."', '".$article_title."', '".$a_reply_message."', '".$time."', 0, 0)";
        $update_article_reply = "update article set article_reply_num = (article_reply_num + 1) where article_id = $article_id";
        $insert_reply_result = mysqli_query($connect, $insert_reply);
        $update_article_reply_result = mysqli_query($connect, $update_article_reply);
        if ($insert_reply_result && $update_article_reply_result) {
            header("Refresh: 0");
        } else {
...
if (isset($_POST['money'])) {
    $update_article_money = mysqli_query($connect, "update article set article_money = (article_money + 1) where article_id = $article_id");
    $update_article_owner_point = mysqli_query($connect, "update sign set point = (point + 1) where user_id = $article_owner_id");
    $update_money_owner_point = mysqli_query($connect, "update sign set point = (point - 1) where user_number = $number");
    if (!$update_article_money || !$update_article_owner_point || !$update_money_owner_point) {
...
if (isset($_POST['fav'])) {
    $article_number_title = $number . "_" . $article_title;
    $row_num = mysqli_num_rows(mysqli_query($connect, "select * from fav_article where article_number_title = '".$article_number_title."'"));
    if ($row_num == 0) {
        $article_time_fav = date("Y-m-d H:i:s", mktime());
        $insert_article_fav = mysqli_query($connect, "insert into fav_article (fav_article_owner_number, article_title, article_number_title, article_owner_id, article_frequency, fav_article_time) VALUES ('".$number."', '".$article_title."', '".$article_number_title."', '".$article_owner_id."', 0, '".$article_time_fav."')");
        $update_article_fav_num = mysqli_query($connect, "update article set article_fav_num = (article_fav_num + 1) where article_title = '".$article_title."' and article_owner_id = '".$article_owner_id."'");
        if (!$insert_article_fav || !$update_article_fav_num) {
            die("文章收藏失败：" . mysqli_error($connect));
        } else {
            header("Refresh: 0");
        }
    }
...
<main id="tt-pageContent">
    <div class="container">
        <div class="tt-single-topic-list">
                ...
                <div class="tt-single-topic">
                    <div class="tt-item-header">
                        <div class="tt-item-info info-top">
                            <div class="tt-avatar-icon tt-size-md">
                                <?php
                                if ($article_owner_image_path == "./user_image/") {?>
                                    <svg class="tt-icon">
                                        <use xlink:href="#icon-ava-d"></use>
                                    </svg>
                                <?php } else { ?>
                                    <img src="<?php echo $article_owner_image_path;  ?>" alt="" class="image">
                                <?php } ?>
                            </div>
                            <div class="tt-avatar-title">
                                <a href="user.php"><?php echo $article_owner_name; ?></a>
                            </div>
                    ...
                    <form action="" method="post">
                        <div class="tt-item-info info-bottom">
                            <button class="tt-icon-btn" name="score" type="submit" title="点赞">
                                ...
                                <span class="tt-text"><?php echo $score; ?></span>
                            </button>
                            <button class="tt-icon-btn" name="dis_score" type="submit" title="踩">
                                <i class="tt-icon"><svg><use xlink:href="#icon-dislike"></use></svg></i>
                                <span class="tt-text"><?php echo $dis_score; ?></span>
                            </button>
            ...
            <?php
            $select_reply = "select * from a_reply where article_id = $article_id order by `a_reply_score` desc ";
            ...
            if (!$select_reply_result) {
                ...
                while ($row = mysqli_fetch_assoc($select_reply_result)) {
                    $a_reply_id = $row['a_reply_id'];
                    $a_reply_owner_name = $row['a_reply_owner_name'];
                    $a_reply_owner_id = $row['a_reply_owner_id'];
                    $a_reply_owner_image_path = mysqli_fetch_assoc(mysqli_query($connect, "select image_path from user where id = '".$a_reply_owner_id."'"))['image_path'];
                    $a_reply_content = $row['a_reply_content'];
                    $a_reply_time = $row['a_reply_time'];
                    $a_reply_score = $row['a_reply_score'];
                    $a_reply_dis_score = $row['a_reply_dis_score'];
                    ?>
                    <div class="tt-item">
                        <div class="tt-single-topic">
                            <div class="tt-item-header pt-noborder">
                                <div class="tt-item-info info-top">
                                    <div class="tt-avatar-icon tt-size-md">
                                        <?php
                                        if ($a_reply_owner_image_path == "./user_image/") {?>
                                            <svg class="tt-icon">
                                                <use xlink:href="#icon-ava-d"></use>
                                            </svg>
                                        <?php } else { ?>
                                            ...
                                            <img src="<?php echo $a_reply_owner_image_path;  ?>" alt="" class="image">
                                        <?php } ?>
                                    </div>
                                    <div class="tt-avatar-title">
                                        <a href="#">
                            ...
                            <div class="tt-item-info info-bottom">
                                <a href="a_reply_score.php?a_reply_id=<?php echo $a_reply_id; ?>&article_title=<?php echo $article_title; ?>&judge=1&article_owner_id=<?php echo $article_owner_id;?>" class="tt-icon-btn">
                                    <i class="tt-icon"><svg><use xlink:href="#icon-like"></use></svg></i>
                                    <span class="tt-text"><?php echo $a_reply_score; ?></span>
                                </a>
                                <a href="a_reply_score.php?a_reply_id=<?php echo $a_reply_id; ?>&article_title=<?php echo $article_title; ?>&judge=0&article_owner_id=<?php echo $article_owner_id;?>" class="tt-icon-btn">
                                    <i class="tt-icon"><svg><use xlink:href="#icon-dislike"></use></svg></i>
                                    <span class="tt-text"><?php echo $a_reply_dis_score; ?></span>
                                </a>
                            </div>
                        </div>
                    </div>
            ...
            <div class="tt-wrapper-inner">
                <h4 class="tt-title-separator"><span>以上评论</span></h4>
            </div>

            <div class="tt-wrapper-inner">
                <div class="pt-editor form-default">
                    <h6 class="pt-title">发布您的评论</h6>
                    <form action="" method="post">
                        <div class="form-group">
                            <textarea name="message" class="form-control" rows="6" placeholder="请输入您的评论"></textarea>
                        </div>
