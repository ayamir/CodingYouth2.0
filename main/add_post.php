...
$user_info = mysqli_fetch_assoc(mysqli_query($connect, "select * from user where number = '".$number."'"));
$image_path = $user_info['image_path'];
$identity = $user_info['identity'];
if (isset($_POST['addPost'])) {
    if (!empty($_POST['title']) && !empty($_POST['message']) && !empty($_POST['category'])) {
        $category = $_POST['category'];
        if ($connect->connect_error) {
            die("数据库连接错误：" . mysqli_error($connect));
        } else {
            $sql_select = "select * from  codingyouth.user where number = $number";
            $sql_select_result = mysqli_fetch_assoc(mysqli_query($connect, $sql_select));
            if ($sql_select_result) {
                $id = $sql_select_result['id'];
                $name = $sql_select_result['name'];
            } else {
                die("数据库查询错误：" . mysqli_error($connect));
            }
            $title = $_POST['title'];
            $content = $_POST['message'];
            $time = date("Y-m-d H:i:s", mktime());
            if ($category == 'post') {
                $post_point = $_POST['choosePoints'];
                $post_tag = $_POST['tag'];
                $insert_post = "insert into codingyouth.post (`post_owner_id`, `post_owner_name`, `post_title`, `post_content`, `post_tag`, `post_reply_num`, `post_score`, `post_money`, `post_point`, `post_time`, `post_fav_num`) 
                values ('".$id."', '".$name."', '".$title."', '".$content."', '".$post_tag."', 0, 0, 0, '".$post_point."', '".$time."', 0)";
                $insert_post_result = mysqli_query($connect, $insert_post);
                $update_user_post = "update user set post_num = (`post_num` + 1) where number = $number";
                $update_user_post_result = mysqli_query($connect, $update_user_post);
                $update_user_exp_p = "update user set exp = (exp + 5) where number = $number";
                $update_user_exp_p_result = mysqli_query($connect, $update_user_exp_p);
                if ($insert_post_result && $update_user_post_result && $update_user_exp_p_result) {
                    echo "<script>alert('发帖成功，将跳转个人中心')</script>";
                    echo "<script>window.location.href='user.php'</script>";
                } else {
                    die(mysqli_error($connect));
                }
            } else if ($category == 'article') {
                $article_tag = $_POST['tag'];
                $insert_article = "insert into codingyouth.article (`article_owner_id`, `article_owner_name`, `article_title`, `article_content`, `article_tag`, `article_reply_num`, `article_score`, `article_dis_score`, `article_money`, `article_time`, `article_fav_num`) 
                values ('".$id."', '".$name."', '".$title."', '".$content."', '".$article_tag."', 0, 0, 0, 0, '".$time."', 0)";
                $insert_article_result = mysqli_query($connect, $insert_article);
                if (!$insert_article_result) {
                    die(mysqli_error($connect));
                }
                $update_user_article = "update user set article_num = (article_num + 1) where number = $number";
                $update_user_article_result = mysqli_query($connect, $update_user_article);
                $update_user_exp_a = "update user set exp = (exp + 10) where number = $number";
                $update_user_exp_a_result = mysqli_query($connect, $update_user_exp_a);
                if ($insert_article_result && $update_user_article_result && $update_user_exp_a_result) {
                    echo "<script>alert('文章发表成功，将跳转个人中心')</script>";
                    echo "<script>window.location.href='user.php'</script>";
                } else {
                    die(mysqli_error($connect));
                }
        ...
        echo "<script>alert('请检查输入内容是否完整')</script>";
    }
}

if (isset($_POST['upload_submit'])) {
    if (!empty($_POST['video_title']) && !empty($_POST['video_desc']) && !empty($_POST['video_content'])) {
        $allowedExts = array("avi", "mp4", "wmv", "asf", "asx", "3gp", "mov", "m4v");
        $allowedCovExts = array("gif", "jpeg", "jpg", "png", "svg", "bmp", "x-png");
        $temp = explode(".", $_FILES["file"]["name"]);
        $tempCov = explode('.', $_FILES['cover']['name']);
        $extension = end($temp);     // 获取文件后缀名
        $extensionCov = end($tempCov);
        if ((   ($_FILES["file"]["type"] == "video/avi")
                || ($_FILES["file"]["type"] == "video/mp4")
                || ($_FILES["file"]["type"] == "video/wmv")
                || ($_FILES["file"]["type"] == "video/asf")
                || ($_FILES["file"]["type"] == "video/asx")
            ...
            && in_array($extensionCov, $allowedCovExts)
        ) {
            if (($_FILES["file"]["size"] < 209715200)   // 小于 200 mb
            && ($_FILES["cover"]["size"] < 20971520)) {
                if ($_FILES["file"]["error"] > 0 || $_FILES["cover"]["error"] > 0) {
                    echo "错误：" . $_FILES["file"]["error"] . "<br>";
                    echo "错误：" . $_FILES["cover"]['error'] . "<br>";
                } else {
                    if (file_exists("upload_video/" . str_replace(' ', '_',  $_FILES["file"]["name"]))
                        || file_exists("upload_cover/" . str_replace(' ', '_', $_FILES['cover']['name']))
                    ) {
                        echo "<script>alert('文件已经存在，请勿重复上传')</script>";
                    } else {
                        $video_path = 'upload_video/';
                        $cover_path = 'upload_cover/';
                        $video_name = str_replace(' ', '_', $_FILES["file"]["name"]);
                        $cover_temp = explode("/", $_FILES['cover']['type']);
                        $cover_type = end($cover_temp);
                        $cover_name = substr_replace($video_name, $cover_type, -3);
                        $copy_result = copy($_FILES["file"]["tmp_name"], $video_path . $video_name);
                        $copy_cover_result = copy($_FILES['cover']['tmp_name'], $cover_path .$cover_name);
                        if($copy_result) {
                            if ($connect->connect_error) {
                                ...
                                $select_id = "select id from user where number = $number";
                                $select_name = "select name from user where number = $number";
                                $id = mysqli_fetch_assoc(mysqli_query($connect, $select_id));
                                $name = mysqli_fetch_assoc(mysqli_query($connect, $select_name));
                                $time = date("Y-m-d H:i:s", mktime());

                                // 长度问题
                                $getid3 = new getID3();
                                $video_info = $getid3->analyze("$video_path" . "$video_name");
                                $video_length = round($video_info['playtime_seconds']);
                                if ($video_length < 10) {
                                    $video_duration = "0:0" . "$video_length";
                                } else if ($video_length < 60) {
                                    $video_duration = "0:" . "$video_length";
                                } else {
				    ...
				    if ($minutes < 10) {
				    	$minutes = "0" . $minutes;
                                ...
                                // 视频封面
                                // 插入数据库
                                $insert_video = "insert into codingyouth.video (`video_owner_id`, `video_owner_name`, `video_name`, `video_title`, `video_desc`, `video_content`, `video_tag`, `video_duration`, `video_path`, `cover_path`, `video_reply_num`, `visit_num`, `video_score`, `video_dis_score`, `video_money`, `video_time`, `video_fav_num`)
                                        values ('".$id['id']."', '".$name['name']."', '".$video_name."', '".$video_title."', '".$video_desc."', '".$video_content."', '".$video_tag."', '".$video_duration."', '".$video_path."', '".$cover_name."', 0, 0, 0, 0, 0, '".$time."', 0)";
                                $insert_cover = "insert into codingyouth.cover (cover_name, cover_path, cover_time) values ('".$cover_name."', '".$cover_path."', '".$time."')";
                                $insert_video_result = mysqli_query($connect, $insert_video);
                                $insert_cover_result = mysqli_query($connect, $insert_cover);
                                if ($insert_video_result && $insert_cover_result) {
                                    echo "<script>alert('上传成功')</script>";
                                } else if (!$insert_cover_result){
                                    echo "<script>alert('封面信息插入数据库失败')</script>";
                                    die(mysqli_error($connect));
                                } else if (!$insert_video_result) {
                                    echo "<script>alert('视频信息插入数据库失败')</script>";
                            ...
                            echo "文件上传失败了，这应该不是您的原因";
                        }
                    }
                }
            } else {
                echo "<script>alert('请确保视频小于200MB，封面小于20MB')</script>";
            }
        } else {
            echo "<script>alert('请检查文件格式')</script>";
        }
    } else {
        echo "<script>alert('请检查内容是否填写完整')</script>";
    }
}
...
<head>
    ...
    <link rel="stylesheet" href="../page-upload-css/layui.css" media="all">
    <meta charset="utf-8">
    <title>发布新内容</title>
            ...
            <div class="tab-content">
                <div class="tt-layout-tab">
                    <div class="tt-wrapper-inner">
                        ...
                        <form class="form-default form-create-topic" action="" method="post" enctype="multipart/form-data">
                            <?php if ($identity == 1) { ?>
                            <div class="form-group">
                                <h6 class="pt-title">视频上传</h6>
                                    <div class="layui-upload">
                                        <div>
                                            <label for="upload_video">视频文件：<br>
                                                <input type="file" name="file" id="upload_video" accept="video/*">
                                            </label><br>
                                            <label for="video_cover">视频封面：<br>
                                                <input type="file" name="cover" id="video_cover" accept="image/*">
                                            </label>
                                            <div class="form-group">
                                                <label for="video_desc">视频标题</label>
                                                <div class="tt-value-wrapper">
                                                    <input type="text" name="video_title" id="video_desc" class="form-control">
                                                    <span class="tt-value-input">50</span>
                                                </div>
                                            </div>

                                            <div class="pt-editor">
                                                <h6 class="pt-title">视频简介</h6><br>
                                                <div class="form-group">
                                                    <textarea name="video_desc" class="form-control" rows="10" placeholder="请简要介绍您的视频内容"></textarea>
                                                </div>
                                            </div>
                                            <div class="pt-editor">
                                                <h6 class="pt-title">视频主要的知识点</h6><br>
                                                <div class="form-group">
                                                    <textarea name="video_content" class="form-control" rows="10" placeholder="请介绍视频所涉及的主要知识点"></textarea>
                                                </div><br>
                                            </div>
                                            <label for="tag">内容分类</label>
                                            <select class="form-control" id="video_tag" name="video_tag">
                                        ...
                                        <div style="text-align: right">
                                            <button type="submit" class="layui-btn" name="upload_submit">上传视频</button>
                                        </div>
                                    </div>
                            </div>
                            <?php } ?>
                            ...
                            <div class="form-group">
                                ...
                                <div class="tt-value-wrapper">
                                    <input type="text" name="title" class="form-control" id="postTitle" placeholder="请尽可能简练的描述您的主题内容">
                                    <span class="tt-value-input">99</span>
                                </div>
                            </div>

                            <div class="pt-editor">
                                <h6 class="pt-title">主题内容</h6><br>
                                <div class="form-group">
                                    <textarea style="height: 800px" name="message" class="form-control" placeholder="开始您的创作"></textarea>
                                </div><br>
                                <div class="row">
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            <label for="category">分类</label>
                                            <select class="form-control" id="category" name="category" onchange="show(this[selectedIndex].value)">
                                                <option  value="article">文章</option>
                                                <option  value="post" id="post" selected>问题</option>
                                            </select>
                                            <label for="tag">内容分类</label>
                                            ...
                                            <script>
                                                function show(vs) {
                                                    if (vs === "post") {
                                                        $("#choosePoints").css("display", "block");
                                                    } else {
                                                        $("#choosePoints").css("display", "none");
                                                    }
                                                }
                                            </script>
                                            <div id="choosePoints">
                                                <label for="choosePoints">设置悬赏佑币数：</label>
                                                <select class="form-control" id="choosePoints" name="choosePoints">
                                                    <option value="0">无悬赏</option>
                                                    <option value="5">5佑币</option>
                                                    <option value="10">10佑币</option>
                                                    <option value="15">15佑币</option>
                                                    <option value="20">20佑币</option>
                                                </select>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="row">
                                    <div class="col-auto ml-md-auto" style="text-align: right">
                                        <input type="submit" name="addPost" class="btn btn-secondary btn-width-lg" value="发布">
                                    </div><br>
                                </div>
                            </div>
                        </form>
...
<script src="../page-upload-js/layui.js" charset="utf-8"></script>
