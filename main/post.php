...
if ($connect->connect_error) {
    ...
    $select_post_owner = mysqli_fetch_assoc(mysqli_query($connect, "select * from user where id = $post_owner_id"));
    $post_owner_name = $select_post_owner['name'];
    $post_owner_image_path = $select_post_owner['image_path'];
    $post_owner_number = $select_post_owner['number'];
    $select = "select * from user where number = $number";
    $res = mysqli_fetch_assoc(mysqli_query($connect, $select));
    if (!$res) {
        die("读取用户信息失败：" . mysqli_error($connect));
    } else {
        $user_id = $res['id'];
        $user_name = $res['name'];
    ...
    $sql_select = "select * from post where post_title = '".$post_title."' and post_owner_id = '".$post_owner_id."'";
    $sql_select_result = mysqli_query($connect, $sql_select);
    if (!$sql_select_result) {
        die("读取帖子信息失败：" . mysqli_error($connect));
    } else {
        $result = mysqli_fetch_assoc($sql_select_result);
        $post_id = $result['post_id'];
        $content = $result['post_content'];
        $reply_num = $result['post_reply_num'];
        $score = $result['post_score'];
        $money = $result['post_money'];
        $point = $result['post_point'];
        $time = $result['post_time'];
        $fav_num = $result['post_fav_num'];
    }
}
?>
...
<head>
    <meta charset="UTF-8">
    <title><?php echo $_GET['post_title']; ?></title>
...
if (isset($_POST['score'])) {
    $sql_update_post_score = "update `post` set `post_score` = (`post_score` + 1) where `post_id` = $post_id";
    $update_post_score_result = mysqli_query($connect, $sql_update_post_score);
    $sql_update_user = "update `user` set `post_score` = (`post_score` + 1) where `number` = $number";
    $update_user_result = mysqli_query($connect, $sql_update_user);
    header("Refresh: 0");
}
...
if (isset($_POST['reply'])) {
    if (!empty($_POST['message'])) {
        ...
        $sql_insert_reply = "insert into reply (post_id, reply_object_id ,reply_owner_id, reply_owner_name, reply_post_title, reply_content, reply_time, reply_score, reply_dis_score, reply_state) 
        VALUES ('".$post_id."', '".$post_owner_id."', '".$user_id."', '".$user_name."', '".$post_title."', '".$reply_message."', '".$time."', 0, 0, 0)";
        $sql_update_post_reply = "update post set post_reply_num = (post_reply_num + 1) where post_id = $post_id";
        $sql_insert_reply_result = mysqli_query($connect, $sql_insert_reply);
        $sql_update_post_reply_result = mysqli_query($connect, $sql_update_post_reply);
        if ($sql_insert_reply_result && $sql_update_post_reply_result) {
            header("Refresh: 0");
        } else {
...
if (isset($_POST['money'])) {
    $update_post_money = "update post set post_money = (post_money + 1) where post_id = '".$post_id."'";
    $update_post_money_result = mysqli_query($connect, $update_post_money);
    $update_post_owner_point = mysqli_query($connect, "update sign set point = (point + 1) where user_id = '".$post_owner_id."'");
    $update_money_owner_point = mysqli_query($connect, "update sign set point = (point - 1) where user_number = '".$number."'");
    if (!$update_post_money_result || !$update_post_owner_point || !$update_money_owner_point) {
        die("帖子赞赏失败：" . mysqli_error($connect));
    } else {
        header("Refresh: 0");
    }
}
...
if (isset($_POST['fav'])) {
    $post_number_title = $number . "_" . $post_title;
    $row_num = mysqli_num_rows(mysqli_query($connect, "select * from fav_post where post_number_title = '".$post_number_title."'"));
    if ($row_num == 0) {
        $post_time_fav = date("Y-m-d H:i:s", mktime());
        $insert_post_fav = mysqli_query($connect, "insert into fav_post (fav_post_owner_number, post_title, post_number_title, post_owner_id, post_frequency, fav_post_time) VALUES ('".$number."', '".$post_title."', '".$post_number_title."', '".$post_owner_id."', 0, '".$post_time_fav."')");
        $update_post_fav_num = mysqli_query($connect, "update post set post_fav_num = (post_fav_num + 1) where post_title = '".$post_title."' and post_owner_id = '".$post_owner_id."'");
        if (!$insert_post_fav || !$update_post_fav_num) {
            die("问题收藏失败：" . mysqli_error($connect));
        } else {
            header("Refresh: 0");
        }
    }
    ...
    <div class="container" >
        <div class="row tt-row no-gutters">
            <div class="col-auto">
                ...
                <div class="tt-search" style="margin-top: 15px">
                    <!-- toggle -->
...
<main id="tt-pageContent">
    <div class="container">
        <div class="tt-single-topic-list">
                ...
                <div class="tt-single-topic">
                    <div class="tt-item-header">
                        <div class="tt-item-info info-top">
                            <div class="tt-avatar-icon tt-size-md">
                                <?php
                                if ($post_owner_image_path == "./user_image/") {?>
                                    <svg class="tt-icon">
                                        <use xlink:href="#icon-ava-d"></use>
                                    </svg>
                                <?php } else { ?>
                                    <img src="<?php echo $post_owner_image_path;  ?>" alt="" class="image">
                                <?php } ?>
                            </div>
                            <div class="tt-avatar-title">
                                <a href="user.php"><?php echo $post_owner_name; ?></a>
                            </div>
                            <div>
                                悬赏佑币数：<?php echo $point; ?>
                            </div>
                    ...
                    <div class="tt-item-description">
                        <?php
                        $eols = array("\r", "\r\n", "\n");
                        $new_content = str_replace($eols, '<br>', $content);
                        ?>
                        <p>
                    ...
                    <form action="" method="post">
                        <div class="tt-item-info info-bottom">
                            <button class="tt-icon-btn" name="score" type="submit" title="点赞">
                                <i class="tt-icon"><svg><use xlink:href="#icon-like"></use></svg></i>
                                <span class="tt-text" title="点赞"><?php echo $score; ?></span>
                            </button>
                            <button class="tt-icon-btn" name="money" type="submit" title="赞赏">
                                <i class="tt-icon"><svg><use xlink:href="#icon-favorite"></use></svg></i>
                                <span class="tt-text"><?php echo $money; ?></span>
                            </button>
                            <button class="tt-icon-btn" name="fav" type="submit" title="收藏">
                                <i class="tt-icon"><svg><use xlink:href="#icon-flag"></use></svg></i>
                                <span class="tt-text"><?php echo $fav_num; ?></span>
                            </button>
                        </div>
                    </form>
                </div>
            </div>
            <HR style="FILTER: alpha(opacity=100,finishopacity=0,style=1)" width="80%" color=#987cb9 SIZE=3><br>
            <?php
            $select_best_reply = mysqli_query($connect, "select * from reply where post_id = $post_id and reply_state = 1 order by `reply_score` desc ");
            $row_num = mysqli_num_rows($select_best_reply);
            if ($row_num != 0) {
                while ($row = mysqli_fetch_assoc($select_best_reply)) {
                    $best_reply_id = $row['reply_id'];
                    $best_reply_owner_name = $row['reply_owner_name'];
                    $best_reply_owner_id = $row['reply_owner_id'];
                    $best_reply_owner_image_path = mysqli_fetch_assoc(mysqli_query($connect, "select image_path from user where id = $best_reply_owner_id"))['image_path'];
                    $best_reply_content = $row['reply_content'];
                    $best_reply_time = $row['reply_time'];
                    $best_reply_score = $row['reply_score'];
                    $best_reply_dis_score = $row['reply_dis_score']; ?>
                    <div class="tt-item" style="border-radius:25px; -moz-border-radius:25px; box-shadow: 2px 2px 20px 20px rgba(0, 0, 0, 0.05);">
                        <div class="tt-single-topic">
                            <div class="tt-item-header pt-noborder">
                                <div class="tt-item-info info-top">
                                    <div class="tt-avatar-icon tt-size-md">
                                        <?php
                                        if ($best_reply_owner_image_path == "./user_image/") {?>
                                            <svg class="tt-icon">
                                                <use xlink:href="#icon-ava-d"></use>
                                            </svg>
                                        <?php } else { ?>
                                            <img src="<?php echo $best_reply_owner_image_path;  ?>" alt="" class="image">
                                        <?php } ?>
                                    </div>
                                    <div class="tt-avatar-title">
                                        <a href="#">
                                            <?php echo $best_reply_owner_name; ?>
                                        </a>
                                    </div>
                                    <a href="#" class="tt-info-time">
                                        <i class="tt-icon"><svg><use xlink:href="#icon-time"></use></svg></i>
                                        <?php echo $best_reply_time; ?>
                                    </a>
                                </div>
                            </div>
                            <div class="tt-item-description">
                                <?php echo $best_reply_content; ?>
                            </div>
                            <div class="tt-item-info info-bottom">
                                <a href="reply_score.php?reply_id=<?php echo $reply_id; ?>&post_title=<?php echo $post_title; ?>&judge=1&post_owner_id=<?php echo $post_owner_id; ?>" class="tt-icon-btn">
                                    <i class="tt-icon"><svg><use xlink:href="#icon-like"></use></svg></i>
                                    <span class="tt-text"><?php echo $best_reply_score; ?></span>
                                </a>
                                <a href="reply_score.php?reply_id=<?php echo $reply_id; ?>&post_title=<?php echo $post_title; ?>&judge=0&post_owner_id=<?php echo $post_owner_id; ?>" class="tt-icon-btn">
                                    <i class="tt-icon"><svg><use xlink:href="#icon-dislike"></use></svg></i>
                                    <span class="tt-text"><?php echo $best_reply_dis_score; ?></span>
                                </a>
                            </div>
                            <div align="right">
                                <span class="tt-color20 tt-badge">最佳答案</span>
                            </div>
                        </div>
                    </div>
            <?php
            ...
            <hr>
            <?php
            $sql_select_reply = "select * from reply where post_id = $post_id and reply_state = 0 order by `reply_score` desc ";
            $sql_select_reply_result = mysqli_query($connect, $sql_select_reply);
            if (!$sql_select_reply_result) {
                die("数据库读取失败：" . mysqli_error($connect));
            } else {
                while ($row = mysqli_fetch_assoc($sql_select_reply_result)) {
                    $reply_id = $row['reply_id'];
                    $reply_owner_name = $row['reply_owner_name'];
                    $reply_owner_id = $row['reply_owner_id'];
                    ...
                    $reply_dis_score = $row['reply_dis_score'];
                ...
                <div class="tt-single-topic">
                    <div class="tt-item-header pt-noborder">
                        <div class="tt-item-info info-top">
                            <div class="tt-avatar-icon tt-size-md">
                                <?php
                                if ($reply_owner_image_path == "./user_image/") {?>
                                    <svg class="tt-icon">
                                        <use xlink:href="#icon-ava-d"></use>
                                    </svg>
                                <?php } else { ?>
                                    <img src="<?php echo $reply_owner_image_path;  ?>" alt="" class="image">
                                <?php } ?>
                            </div>
                            <div class="tt-avatar-title">
                                <a href="#">
                    ...
                    <div class="tt-item-info info-bottom">
                        <a href="reply_score.php?reply_id=<?php echo $reply_id; ?>&post_title=<?php echo $post_title; ?>&judge=1&post_owner_id=<?php echo $post_owner_id; ?>" class="tt-icon-btn">
                            <i class="tt-icon"><svg><use xlink:href="#icon-like"></use></svg></i>
                            <span class="tt-text"><?php echo $reply_score; ?></span>
                        </a>
                        <a href="reply_score.php?reply_id=<?php echo $reply_id; ?>&post_title=<?php echo $post_title; ?>&judge=0&post_owner_id=<?php echo $post_owner_id; ?>" class="tt-icon-btn">
                            <i class="tt-icon"><svg><use xlink:href="#icon-dislike"></use></svg></i>
                            <span class="tt-text"><?php echo $reply_dis_score; ?></span>
                        </a>
                    </div>
                    <?php
                    if ($_SESSION['number'] == $post_owner_number && $reply_state == 0) {  ?>
                        <div align="right">
                            <a href="change_reply_state.php?post_owner_id=<?php echo $post_owner_id; ?>&reply_owner_id=<?php echo $reply_owner_id; ?>&point=<?php echo $point; ?>&post_title=<?php echo $post_title; ?>" onclick="this.alert('此操作不可逆，确定设为最佳答案吗？')">设为最佳答案</a>
                        </div>
                    <?php } if ($reply_state == 1) { ?>
                        <div align="right">
                            <span class="tt-color20 tt-badge">最佳答案</span>
                        </div>
                    <?php }?>
            ...
            <div class="tt-wrapper-inner">
                <h4 class="tt-title-separator"><span>以上回答</span></h4>
            </div>

            <div class="tt-wrapper-inner">
                <div class="pt-editor form-default">
                    <h6 class="pt-title">发布您的回答</h6>
                    <form action="" method="post">
                        <div class="form-group">
                            <textarea name="message" class="form-control" rows="6" placeholder="请输入您的回答"></textarea>
                        </div>
                        <div  style="text-align: right" >
                            <div class="col-auto">
                                <form action="" method="post">
                                    <button name="reply" type="submit" class="btn btn-secondary btn-width-lg">评论</button>
                                </form>
                            </div>
                        </div>
                    </form>
