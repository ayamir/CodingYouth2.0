...
if ($connect->error) {
    die("数据库连接错误" . mysqli_error($connect));
} else {
    $user_number = $_SESSION['number'];
    $image_path = mysqli_fetch_assoc(mysqli_query($connect, "select * from user where number = '".$user_number."'"))['image_path'];
    $video_owner_id = $_GET['video_owner_id'];
    $video_owner_image_path = mysqli_fetch_assoc(mysqli_query($connect, "select image_path from user where id = $video_owner_id"))['image_path'];
    $video_title = "" . $_GET['video_title']. "";

    $select_video = "select * from video where video_title = '".$video_title."' and video_owner_id = '".$video_owner_id."'";
    $select_video_result = mysqli_query($connect, $select_video);
    if (!$select_video_result) {
        die("读取视频信息失败：" . mysqli_error($connect));
    } else {
        $select_objects = mysqli_fetch_assoc($select_video_result);
        $video_id = $select_objects['video_id'];
        $video_name = $select_objects['video_name'];
        $video_desc = $select_objects['video_desc'];
        $video_content = $select_objects['video_content'];
        $video_tag = $select_objects['video_tag'];
        $video_owner_name = $select_objects['video_owner_name'];
        $visit_num = $select_objects['visit_num'];
        $video_score = $select_objects['video_score'];
        $video_dis_score = $select_objects['video_dis_score'];
        $video_reply_num = $select_objects['video_reply_num'];
        $video_money = $select_objects['video_money'];
        $video_path = $select_objects['video_path'];
        $video_time = $select_objects['video_time'];
        $video_fav_num = $select_objects['video_fav_num'];
        $cover_path = $select_objects['cover_path'];
        mysqli_query($connect, "update video set visit_num = (visit_num + 1) where video_id = '".$video_id."'");
    }
    if (isset($_POST['reply'])) {
        if (!empty($_POST['content'])) {
            $v_reply_owner_number = $_SESSION['number'];
            $v_reply_owner_res = mysqli_fetch_assoc(mysqli_query($connect, "select * from user where number = '".$v_reply_owner_number."'"));
            $v_reply_owner_id = $v_reply_owner_res['id'];
            $v_reply_owner_name = $v_reply_owner_res['name'];
            $v_reply_content = $_POST['content'];
            $v_reply_time = date("Y-m-d H:i:s", mktime());
            $insert_v_reply = "insert into v_reply (video_id, v_reply_object_id, v_reply_owner_id, v_reply_owner_name, v_reply_video_title, v_reply_content, v_reply_score, v_reply_dis_score, v_reply_time) 
            VALUES ('".$video_id."', '".$video_owner_id."', '".$v_reply_owner_id."', '".$v_reply_owner_name."', '".$video_title."', '".$v_reply_content."', 0, 0, '".$v_reply_time."')";
            $insert_v_reply_result = mysqli_query($connect, $insert_v_reply);
            $update_video_reply = "update video set video_reply_num = (video_reply_num + 1) where video_id = '".$video_id."'";
            $update_video_reply_result = mysqli_query($connect, $update_video_reply);
            if (!$insert_v_reply_result) {
                die("更新评论失败：" . mysqli_error($connect));
            } else if (!$update_video_reply_result) {
                die("更新评论失败2：" . mysqli_error($connect));
            }
        } else {
    ...
    if (isset($_POST['score'])) {
        $update_video_score = "update video set video_score = (video_score + 1) where video_id = '".$video_id."'";
        $update_video_score_result = mysqli_query($connect, $update_video_score);
        if (!$update_video_score_result) {
            die("点赞失败：" . mysqli_error($connect));
        }
    }

    if (isset($_POST['dis_score'])) {
        $update_video_dis_score = "update video set video_dis_score = (video_dis_score + 1) where video_id = '".$video_id."'";
        $update_video_dis_score_result = mysqli_query($connect, $update_video_dis_score);
        if (!($update_video_dis_score_result)) {
            die("踩失败：" . mysqli_error($connect));
        }
    }

    if (isset($_POST['fav'])) {
        $video_number_title = $user_number . "_" . $video_title;
        $row_num = mysqli_num_rows(mysqli_query($connect, "select * from fav_video where video_number_title = '".$video_number_title."'"));
        if ($row_num == 0) {
            $video_time_fav = date("Y-m-d H:i:s", mktime());
            $insert_video_fav = mysqli_query($connect,
                "insert into fav_video (fav_video_owner_number, video_title,  video_number_title, video_owner_id, video_frequency, fav_video_time) VALUES ('".$user_number."', '".$video_title."', '".$video_number_title."', '".$video_owner_id."', 0, '".$video_time_fav."')");
            $update_video_fav_num = mysqli_query($connect, "update video set video_fav_num = (video_fav_num + 1) where video_title = '".$video_title."' and video_owner_id = '".$video_owner_id."'");
            if (!$insert_video_fav || !$update_video_fav_num) {
                die("收藏失败：" . mysqli_error($connect));
            } else {
                header("Refresh: 0");
            }
        }
...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="../page-files-css/fontello.css">

    <link rel="stylesheet" type="text/css" href="../page-single-video-css/animate.css">
    <link rel="stylesheet" type="text/css" href="../page-single-video-css/bootstrap.min.css">
    <link rel="stylesheet" type="text/css" href="../page-single-video-css/thumbs-embedded.css">
    <link rel="stylesheet" type="text/css" href="../page-single-video-css/video-js.css">
    <link rel="stylesheet" type="text/css" href="../page-single-video-css/style.css">
    <link rel="stylesheet" type="text/css" href="../page-single-video-css/responsive.css">
    <link rel="stylesheet" type="text/css" href="../page-single-video-css/color.css">
</head>
...
<header id="tt-header" style="background-color: #f5dc7d; -webkit-border-bottom-left-radius: 15px; -webkit-border-bottom-right-radius: 15px;  box-shadow: 2px 2px 20px 20px rgba(0, 0, 0, 0.15); ">
    <div class="container" >
        <div class="row tt-row no-gutters">
            <div class="col-auto">
                ...
                <div class="tt-search" style="background-color: white">
                    <!-- toggle -->
            ...
            <div class="col-auto ml-auto">
                <div class="tt-user-info d-flex justify-content-center">
                    <div class="tt-avatar-icon tt-size-md">
                        <a href="user.php" class="tt-btn-icon">
                            <?php
...
<div>
    <main style="background-color: white; opacity: 0.9;">
                ...
                <div class="row">
                    <div class="col-lg-9">
                        <div class="mn-vid-sc single_video">
                            <div class="vid-1">
                                <div class="vid-pr" style="border-radius:25px; -moz-border-radius:25px; height: 540px; width: 960px; box-shadow: 2px 2px 20px 20px rgba(0, 0, 0, 0.15);">
                                    <video
                                        id="my-video"
                                        class="video-js"
                                        controls
                                        ...
                                        width="640"
                                        height="360"
                                        poster="upload_cover/<?php echo $cover_path; ?>"
                                        data-setup="{}"
                                    >
                                        <source src="<?php echo $video_path . $video_name; ?>" type="video/mp4"/>
                                    </video>
                                </div>
                                ...
                                <form action="" method="post">
                                    <div class="vid-info">
                                        ...
                                        <div class="info-pr" style="margin-right: 50px">
                                            <span><?php echo $visit_num; ?> views .</span>
                                            <ul class="chan_cantrz">
                                                <li>
                                                    <a href="#" class="subscribe">投币</a>
                                                </li>
                                            </ul>
                                            <ul class="pr_links">
                                                <li>
                                                    <button data-toggle="tooltip" data-placement="top" type="submit" name="score" title="点赞">
                                                        <i class="icon-thumbs_up_fill"></i>
                                                    </button>
                                                    <span><?php echo $video_score; ?></span>
                                                </li>
                                                <li>
                                                    <button data-toggle="tooltip" data-placement="top" type="submit" name="dis_score" title="踩">
                                                        <i class="icon-thumbs_down_fill"></i>
                                                    </button>
                                                    <span><?php echo $video_dis_score; ?></span>
                                                </li>
                                                <li>
                                                    <form action="" method="post">
                                                        <button data-toggle="tooltip" data-placement="top" name="fav" type="submit" title="收藏">
                                                            <i class="icon-flag"></i>
                                                        </button>
                                                        <span><?php echo $video_fav_num; ?></span>
                                                    </form>
                                                </li>
                                            </ul>
                            ...
                            <div class="abt-mk">
                                <div class="info-pr-sec">
                                    <div class="vcp_inf cr">
                                        <div class="vc_hd">
					<?php if ($video_owner_image_path == "./user_image/") {?>
					<img src="../images/logo.jpg" alt="">
					<?php } else { ?>
					<img src="<?php echo $video_owner_image_path; ?>" alt="">
					<?php } ?>
                                        </div>
                                        <div class="vc_info pr">
                                            <h4><?php echo $video_owner_name; ?></h4>
                                            <span><?php echo $video_time; ?></span>
                                        </div>
                                    </div>
                                ...
                                <div class="about-ch-sec">
                                    ...
                                    <div class="abt-rw">
                                        <h4>知识点：</h4>
                                        <ul>
                                            <li><span><?php echo $video_content; ?></span></li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="cmt-bx">
                                <ul class="cmt-pr">
                                    <li><span><?php echo $video_reply_num; ?></span> 评论</li>
                                    <li>
                                ...
                                <div class="vcp_inf pc">
                                    <div class="tt-avatar-icon tt-size-md">
                                        <i class="tt-icon"><svg><use xlink:href="#icon-ava-r"></use></svg></i>
                                    </div>
                                    <form action="" method="post">
                                        <label for="reply"></label>
                                        <input name="content" id="reply" type="text" placeholder="发表评论">
                                        <button type="submit" name="reply">评论</button>
                                    </form>
                                    <div class="clearfix"></div>
                                    <div class="clearfix"></div>
                                </div>
                            ...
                            <?php
                            $select_reply = "select * from v_reply where video_id = '".$video_id."' order by v_reply_score desc";
                            $select_reply_result = mysqli_query($connect, $select_reply);
                            if (!$select_reply_result) {
                                echo("读取评论信息失败：" . mysqli_error($connect));
                            } else {
                                while ($row = mysqli_fetch_assoc($select_reply_result)) {
                                    $reply_id = $row['v_reply_id'];
                                    $reply_owner_id = $row['v_reply_owner_id'];
                                    $reply_owner_image_path = mysqli_fetch_assoc(mysqli_query($connect, "select image_path from user where id = '".$reply_owner_id."'"))['image_path'];
                                    $reply_owner_name = $row['v_reply_owner_name'];
                                    $reply_content = $row['v_reply_content'];
                                    $reply_score = $row['v_reply_score'];
                                    $reply_dis_score = $row['v_reply_dis_score'];
                                    $reply_time = $row['v_reply_time'];
                                    ?>
                                    <ul class="tt-item" style="width: 115%; border-radius:25px; -moz-border-radius:25px; box-shadow: 2px 2px 20px 20px rgba(0, 0, 0, 0.05);" id="comment">
                                        <li>
                                            <div class="vcp_inf">
                                                <div class="vc_hd">
                                                    <!--用户头像-->
						    <?php if ($reply_owner_image_path == "./user_image") {?>
						    <img src="../images/logo.jpg" alt="">
<?php } else { ?>
						    <img src="<?php echo $reply_owner_image_path; ?>" alt="">
<?php } ?>
                                                </div>
                                                <div class="coments">
                                                    <h2><?php echo $reply_owner_name . "   "; ?><small
                                                            class="posted_dt"><?php echo $reply_time; ?></small></h2>
                                                    <p><?php echo $reply_content; ?></p>
                                                    <ul class="cmn-i">
                                                        <li>
                                                            <a href="v_reply_score.php?v_reply_id=<?php echo $reply_id; ?>&video_title=<?php echo $video_title; ?>&judge=1&video_owner_id=<?php echo $video_owner_id?>" title="">
                                                                <i class="icon-thumbs_up"></i>
                                                            </a>
                                                            <span><?php echo $reply_score; ?></span>
                                                        </li>
                                                        <li>
                                                            <a href="v_reply_score.php?v_reply_id=<?php echo $reply_id; ?>&video_title=<?php echo $video_title; ?>&judge=0&video_owner_id=<?php echo $video_owner_id?>" title="">
                                                                <i class="icon-thumbs_down"></i>
                                                            </a>
                                                            <span><?php echo $reply_dis_score; ?></span>
                                                        </li>
                                                    </ul>
                                                    <a href="#" title="">查看所有回复</a>
                                                </div>
                                            </div>
                                        </li>
                                    </ul>
                    ...
                    <div class="col-lg-3" style="left: 120px" >
                        <div class="sidebar">
                            <div class="vidz-prt">
                                <h2 class="sm-vidz">相似视频</h2>
                                <div class="clearfix"></div>
                            </div>
                            <?php
                            $i = 0;
                            $select_video_other = mysqli_query($connect, "select * from video where video_tag = '".$video_tag."' and video_id != '".$video_id."' order by video_score desc ");
                            while ($row = mysqli_fetch_array($select_video_other)) {
                                $i++;
                                if ($i == 4) {
                                    break;
                                }
                                $video_title_other = $row['video_title'];
                                $video_owner_id_other = $row['video_owner_id'];
                                $video_owner_name_other = $row['video_owner_name'];
                                $visit_num_other = $row['visit_num'];
                                $video_time_other = $row['video_time'];
                                $video_path_other = $row['video_path'];
                                $cover_path_other = $row['cover_path'];
                                $video_duration_other = $row['video_duration'];
                            ?>
                            <div class="videoo">
                                <div class="vid_thumbainl">
                                    <a href="video.php?video_title=<?php echo $video_title_other; ?>&video_owner_id=<?php echo $video_owner_id_other; ?>" title="<?php echo $video_title_other; ?>">
                                        <img src="./upload_cover/<?php echo $cover_path_other; ?>" alt="<?php echo $video_title_other; ?>">
                                        <span class="vid-time"><?php echo $video_duration_other; ?></span>
                                ...
                                <div class="video_info">
                                    <h3>
                                        <a href="video.php?video_title=<?php echo $video_title_other; ?>&video_owner_id=<?php echo $video_owner_id_other; ?>" title="<?php echo $video_title_other?>"></a>
                                    </h3>
                                    <h4>
                                        <a href="video.php?video_title=<?php echo $video_title_other; ?>&video_owner_id=<?php echo $video_owner_id_other; ?>">
                                            <?php echo $video_owner_name_other; ?>
                                        </a>
                                        <span class="verify_ic">
                                            <i class="icon-tick"></i>
                                        </span>
                                    </h4>
                                    <span><?php echo $visit_num_other; ?>  Views.
                                        <small class="posted_dt">
                                        <?php echo $video_time_other; ?>
                                    </small>
                                    </span>
                                </div>
                            </div>
...
<script src="../page-single-video-js/jquery.min.js"></script>
<script src="../page-single-video-js/popper.js"></script>
<script src="../page-single-video-js/bootstrap.min.js"></script>
<script src="../page-single-video-js/video.js"></script>
<script src="../page-single-video-js/script.js"></script>
