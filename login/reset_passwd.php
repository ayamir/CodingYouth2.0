...
$numberErr = $passwordErr = $repasswordErr = "";
...
if (isset($_POST['submit'])) {
    if ($_POST['number'] && $_POST['password'] && $_POST['repassword']) {
        if ($connect->connect_error) {
            echo "<script>alert('数据库连接失败,请稍候再试')</script>";
        } else {
            mysqli_set_charset($connect, "utf8");
            $sql_check = "select number from user where number={$_POST['number']}";
            $result_check = mysqli_query($connect, $sql_check);
            $rowNum = mysqli_num_rows($result_check);
            if (!$rowNum) {
                echo "<script>alert('该账号不存在, 请重新输入')</script>";
            } else {
                $number = $_POST['number'];
                $password = $_POST['password'];
                $passwordS = md5(md5($password));
                $sql_update = "UPDATE `user` SET `password`='".$passwordS."' WHERE number='$number'";
                $result_update = mysqli_query($connect, $sql_update);
                if ($result_update) {
                    echo "<script>alert('修改成功, 点击确认使用新密码重新登录')</script>";
                    echo "<script>window.location.href='../login/login.php'</script>";
                } else {
                    echo mysqli_error($connect);
                    echo "<script>alert('系统忙,请稍后重试!');</script>";
                }
            }
            mysqli_close($connect);
        }
    } else {
        echo "<script>alert('您好像漏填了哦,请检查输入')</script>";
    }
}
?>
<style>
    body {
        background: url(../images/login_background1.jpg) no-repeat center center;
...
<main id="tt-pageContent" class="tt-offset-none">
    <div class="container" style="opacity: 0.95">
        <div class="tt-loginpages-wrapper">
            <div class="tt-loginpages" align="center" style="border-radius:25px; -moz-border-radius:25px; box-shadow: 2px 2px 20px 20px rgba(0, 0, 0, 0.15); height: 700px; ">
                ...
                <form class="form-default" action="" method="post" onsubmit="return on_submit();" style="width: 300px; ">
                    <div class="form-group" style="border-radius:25px; -moz-border-radius:25px;">
                        <label for="userNumber"></label>
                        <input type="text" name="number" id="userNumber" class="form-control" placeholder="手机号" style="border-radius:25px; -moz-border-radius:25px; height: 60px; background-color: white; border: 2px solid; ">
                    </div>
                    <div class="form-group">
                        <label for="userPassword"></label>
                        <input type="password" name="password" id="userPassword" class="form-control" onkeyup="pwStrength(this.value)" onblur="pwStrength(this.value)" placeholder="密码" style="border-radius:25px; -moz-border-radius:25px; height: 60px; background-color: white; border: 2px solid">
                    </div>
                    <div>
                        密码强度:
                        <table border="1" cellpadding="1" bordercolordark="#fdfeff" bordercolorlight="#99ccff"
                               cellspacing="1" style="width: 300px; height: 10px display: inline; background-color: #e0f0ff">
                            <tr>
                                <td id="strength_L" style="width: 100px; height: 10px;" align="center">
                                    弱
                                </td>
                                <td id="strength_M" style="width: 100px; height: 10px;" align="center">
                                    中
                                </td>
                                <td id="strength_H" style="width: 100px; height: 10px;" align="center">
                                    强
                                </td>
                            </tr>
                        </table>
                    ...
                    <br>
                    <div class="form-group">
                        <label for="userRePassword"></label>
                        <input type="password" name="repassword" id="userRePassword" class="form-control" onpaste="return false" oncontextmenu="return false" placeholder="重复密码" style="border-radius:25px; -moz-border-radius:25px; height: 60px; background-color: white; border: 2px solid">
                    </div><br>
                    <button type="submit" name="submit" id="login_control" class="btn btn-secondary btn-block" style="border-radius:25px; -moz-border-radius:25px; height: 50px">确认</button>
                </form>
            </div>
        </div>
    </div>
